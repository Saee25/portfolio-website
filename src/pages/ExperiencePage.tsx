        ...
        <div className="space-y-12">
          {experience.map((exp, i) => {
            ...
            return (
              ...
              >
                <div className={`grid gap-8 ${textRight ? "md:grid-cols-[300px_1fr]" : "md:grid-cols-[1fr_300px]"}`}>
                  <div
                    data-placeholder={`exp-page-image-${i}`}
                    className={`aspect-video md:aspect-square rounded-xl border border-dashed border-primary/30 flex items-center justify-center text-xs text-muted-foreground uppercase tracking-widest ${textRight ? "md:order-1" : "md:order-2"}`}
                  >
                    Add image / logo
                  </div>
                  <div className={textRight ? "md:order-2" : "md:order-1"}>
